...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    # Select the dataset #
    ######################
    dataset = flow_dataset.get_split(FLAGS.dataset_split_name, FLAGS.dataset_dir)
    ...
    image_a = image_preprocessing_fn(image_a, eval_image_size, eval_image_size)
    image_b = image_preprocessing_fn(image_b, eval_image_size, eval_image_size)

    image_a, image_b, labels = tf.train.batch(
    ...
    # Define the model #
    ####################
    logits, _ = network_fn(image_a, image_b)
    ...
    names_to_values, names_to_updates = slim.metrics.aggregate_metric_map({
        ...
        #'Kp': tf.metrics.sparse_precision_at_k(predictions, labels, 5, class_id = 5),
        #'Kr': slim.metrics.streaming_sparse_recall_at_k(logits, labels, 5, class_id = 5),
    })
