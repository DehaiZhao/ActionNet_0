...
name = 'java_3_0.mkv'
output_path = '/home/cheer/video_test/corre/diff'
parts = ['t0', 't1']
...
def make_dirs():
  for part in parts:
    if not os.path.exists(os.path.join(output_path, name, part)):
      print 'create folder {}'.format(name)
      os.makedirs(os.path.join(output_path, name, part))
    else:
...
def start():
  ...
  while(cap.isOpened()):
    ...
    if len(box_max):
      ...
      #cv2.imwrite(os.path.join(output_path, name, parts[0], '{:05}'.format(pos) + '.jpg'), frameB)
      #cv2.imwrite(os.path.join(output_path, name, parts[1], '{:05}'.format(pos) + '.jpg'), diff)
      #cv2.imwrite(os.path.join(output_path, name, parts[2], '{:05}'.format(pos) + '.jpg'), thresh)
      cv2.imwrite(os.path.join(output_path, name, parts[0], '{:05}'.format(pos) + '.jpg'), frameA[overlap[1]:overlap[3], overlap[0]:overlap[2]])
      cv2.imwrite(os.path.join(output_path, name, parts[1], '{:05}'.format(pos) + '.jpg'), frameD[overlap[1]:overlap[3], overlap[0]:overlap[2]])
      box_o = box_max[0:4]
